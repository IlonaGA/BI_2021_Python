...
# def quality_threshold(line, min_quality=0):
#     return sum(map(ord, line)) / len(line) - ord('!')
...
def main(input_fastq, output_file_prefix, gc_bounds = (0, 100),
    ...
    for i in range(0, len(file), 4):
        if check_gc_bounds(file[i + 1], gc_bounds[0], gc_bounds[1]) and \
            check_length_bounds(file[i + 1], length_bounds[0], length_bounds[1]) and \
            check_quality_threshold(file[i + 1], quality_threshold):
